...
function plot(em::ErgodicManager, xd::VVF, vtm::VTM;
              ...
              cmap="Greys",
              show_score::Bool=true,
              c::Vector{String}=ones(String, length(vtm)),
              lw::Float64=1.0,
              ms::Float64=8.0,
              no_domain::Bool=false
             )
    ...
    # if no colors are defined, just make them all blue
    if c[1] == ""
        for j = 1:num_agents
            c[j] = "b"
        end
    ...
    for j = 1:num_agents
        xd = xds[j]
        plot_trajectory(xd, c=c[j], lw=lw, ms=ms)
    end
...
function plot(em::ErgodicManagerR2T, xd::VVF, n::Int;
    ...
    # plotting the trajectory
    dims = 2
    plot_trajectory(xd[1:n], lw=lw, mfc=mfc, ms=ms, dims=dims)
